...
describe("transactionMapper", () => {
  ...
  describe("mapTransactionListItem", () => {
    it("transforms transaction into transaction list item format", () => {
      expect(mapTransactionListItem(transaction, 100)).toEqual({
        blockHash: "0xdfd071dcb9c802f7d11551f4769ca67842041ffb81090c49af7f089c5823f39c",
        blockNumber: "20",
        confirmations: "80",
        contractAddress: "0xc7e0220d02d549c4846A6EC31D89C3B670Ebe35E",
    ...
    it("sets isError as 1 when transaction is failed", () => {
      expect(
        mapTransactionListItem({ ...transaction, status: TransactionStatus.Failed } as Transaction, 100).isError
      ).toBe("1");
    });

    it("sets isError as 0 when transaction is included", () => {
      expect(
        mapTransactionListItem({ ...transaction, status: TransactionStatus.Included } as Transaction, 100).isError
      ).toBe("0");
    });

    it("sets isL1Originated as 1 when transaction is originated from L1", () => {
      expect(mapTransactionListItem({ ...transaction, isL1Originated: true } as Transaction, 100).isL1Originated).toBe(
        "1"
      );
    });

    it("sets isL1Originated as 0 when transaction is not originated from L1", () => {
      expect(mapTransactionListItem({ ...transaction, isL1Originated: false } as Transaction, 100).isL1Originated).toBe(
        "0"
      );
    });
  });
