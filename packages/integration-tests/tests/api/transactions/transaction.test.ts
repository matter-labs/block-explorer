...
describe("Transactions", () => {
  ...
  describe("Transactions for the /address/{address} endpoints", () => {
    jest.setTimeout(localConfig.extendedTimeout);
    ...
    it("Verify the transaction via /address/{address}/transfers", async () => {
      contract = await helper.getStringFromFile(bufferRoute + Buffer.paymaster);
      emptyWallet = await helper.getStringFromFile(bufferRoute + Buffer.emptyWalletAddress);
      txHash = await helper.getStringFromFile(bufferRoute + Buffer.paymasterTx);

      const customTokenAddress = await helper.getStringFromFile(bufferRoute + Buffer.customToken);
