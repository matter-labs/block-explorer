  ...
  describe("/address/{address}/logs", () => {
    ...
    it("Verify the transaction via /address/{address}/logs", async () => {
      ...
      return request(environment.blockExplorerAPI)
        ...
        .expect((res) => expect(res.body.items[0].blockNumber).toBeGreaterThanOrEqual(1))
        ...
        .expect((res) => expect(res.body.items[0]).toStrictEqual(expect.objectContaining({ logIndex: 1 })))
