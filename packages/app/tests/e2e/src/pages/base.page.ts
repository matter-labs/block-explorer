...
export class BasePage {
  ...
  async click(element: any, testId?: boolean) {
    ...
    await this.world.page
      ?.locator(selector)
      .first()
      .click({ force: true, timeout: config.increasedTimeout.timeout });
  }
  ...
  async dblClick(element: any) {
    await this.world.page
      ?.locator(element)
      .first()
      .dblclick({ force: true, timeout: config.increasedTimeout.timeout });
  }
  ...
  async getElementByTextInsideALogTab(text: string) {
    element = await this.world.page
      ?.locator(`//*[not(self::th)][text()='${text}'][1]`)
      .first();
    return await element;
  }
  ...
  async getElementByPartialText(text: string) {
    element = await this.world.page
      ?.locator(`//*[contains(text(),'${text}')][1]`)
      .first();
    return await element;
  }

  async getElementByPartialHref(partialHref: string) {
    element = await this.world.page?.locator(
      `//*[contains(@href,'${partialHref}')]`,
    );
  ...
  async getElementByHrefAndText(partialHref: string, text: string) {
    element = await this.world.page
      ?.locator(`//*[text()='${text}' and contains(@href,'${partialHref}')]`)
      .first();
    await element.scrollIntoViewIfNeeded();
    return await element;
  }
  ...
  async getElementByTestIdAndText(testid: string, text: string) {
    element = await this.world.page?.locator(
      `//*[@${this.byTestId}'${testid}' and contains(text(), '${text}')]`,
    );
    await element.scrollIntoViewIfNeeded();
    return await element;
  }
