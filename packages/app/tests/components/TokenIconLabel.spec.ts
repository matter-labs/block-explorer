...
describe('TokenIconLabel', () => {
  ...
  it('renders custom token icon if iconUrl is not defined', () => {
    ...
    expect(wrapper.find('img')?.attributes('src')).toBe(
      '/images/currencies/customToken.svg',
    );
  });
  ...
  it("renders 'unknown' symbol when it is not exist", () => {
    ...
    expect(wrapper.find('.token-symbol').text()).toBe(
      i18n.global.t('balances.table.unknownSymbol'),
    );
  });
  it('renders correct link of contract page', () => {
    ...
    expect(contractPageUrl[0].props().to.params.address).toBe(
