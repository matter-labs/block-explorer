...
describe('NetworkSwitch:', () => {
  ...
  it('renders listbox button with selected option', async () => {
    ...
    expect(container.querySelector('.network-item-label')?.textContent).toBe(
      'Testnet',
    );
  });
  ...
  it('renders list of networks when button is clicked', async () => {
    ...
    expect(options[0].querySelector('img')?.getAttribute('alt')).toBe(
      'Testnet logo',
    );
    expect(options[1].getAttribute('href')).toBe(
      'https://testnet-beta.staging-scan-v2.zksync.dev/',
    );
