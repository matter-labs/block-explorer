...
describe('InfoTable', () => {
  ...
  it('renders table based on input', () => {
    ...
    expect(address[1].text()).toBe(
      '0xc2675AE7F35b7d85Ed1E828CCf6D0376B01ADea3',
    );

    const creator = rowArray[1].findAll('td');
    expect(creator[0].text()).toBe('Creator');
    const [creatorAddressLink, creatorTxHash] =
      creator[1].findAllComponents(RouterLinkStub);
    expect(creatorAddressLink).toBeTruthy();
    expect(creatorTxHash).toBeTruthy();
    expect(creatorAddressLink.props().to.name).toBe('address');
    expect(creatorAddressLink.props().to.params.address).toBe(
      '0x06590AD9b721DD3d4fd2177d643799E552437904',
    );
