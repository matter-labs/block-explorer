...
const contract: Contract = {
  ...
  creatorTxHash:
    '0xcdab4a39d32a15bafb0b992da1dff8a4b782be450be63c8a64c955758370574f',
  ...
  verificationInfo: {
    artifacts: {
      ...
      bytecode: [
        0, 0, 0, 46, 4, 0, 0, 65, 0, 0, 0, 0, 1, 65, 1, 111, 0, 0, 0, 47, 4, 0,
        0, 65, 0, 0, 0, 0, 0, 20, 3, 118, 0, 0, 0, 48, 1, 0, 0, 65, 0, 0, 0, 0,
        0, 33, 3, 118, 0, 0, 0, 1, 1, 48, 1, 143, 0, 0, 0, 0, 1, 1, 0, 75, 0, 0,
        0, 17,
      ],
    },
    request: {
...
describe('Contract Info Tab', () => {
  ...
  it('renders tabs based on input', () => {
    ...
    expect(wrapper.findAll('.function-disclosure-btn')[0].text()).toBe(
      '1. get',
    );
    expect(wrapper.findAll('.function-type-title')[1].text()).toBe('Write');
    expect(wrapper.findAll('.function-disclosure-btn')[1].text()).toBe(
      '1. increment',
    );
  });
  ...
  it('does not render tabs component', () => {
    ...
    const routerArray = wrapper
      .find('.contract-link-container')
  ...
  describe('Proxy contract', () => {
    describe('when implementation contract is not verified', () => {
      it('renders proxy tabs and contract not verified warning', () => {
        ...
        expect(wrapper.findAll('.tab-head li .tab-btn')[3].text()).toBe(
          'Read as Proxy',
        );
        expect(
          wrapper
            .findAll('.proxy-implementation-link')[0]
            .findComponent(Alert)
            .props().type,
        ).toBe('warning');
        expect(wrapper.findAll('.tab-head li .tab-btn')[4].text()).toBe(
          'Write as Proxy',
        );
        expect(
          wrapper
            .findAll('.proxy-implementation-link')[1]
            .findComponent(Alert)
            .props().type,
        ).toBe('warning');
      });
    ...
    describe('when implementation contract is verified', () => {
      it('renders proxy tabs and read/write as proxy functions', () => {
        ...
        expect(wrapper.findAll('.function-disclosure-btn')[2].text()).toBe(
          '1. get2',
        );
        expect(
          wrapper
        ...
        expect(wrapper.findAll('.function-disclosure-btn')[3].text()).toBe(
          '1. increment2',
        );
      });
    });
