...
describe('FunctionForm:', () => {
  ...
  it('renders form fields based on input', async () => {
    ...
    expect(fields[0].findComponent(FunctionArrayParameter).props().label).toBe(
      'spender',
    );
    expect(fields[0].findComponent(FunctionArrayParameter).props().type).toBe(
      'address[]',
    );
  ...
  it('validates input and shows an error when value is invalid', async () => {
    ...
    await arrayField
      .find('input')
      .setValue('0xa1cf087DB965Ab02Fb3CFaCe1f5c63935815f044');
