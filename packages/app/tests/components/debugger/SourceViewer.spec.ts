...
import SourceViewer, {
  ITEM_HEIGHT,
} from '@/components/debugger/SourceViewer.vue';
...
describe('SourceViewer:', () => {
  ...
  it('collapses a function', async () => {
    const { findByText, findByTestId, queryByText, unmount } = render(
      SourceViewer,
      {
        props: {
          address: '0x00',
  ...
  it('renders list items that are in view port', async () => {
    const source = ['foo', '.func_begin', 'Hello', 'World', '!', '.func_end'];
    const { getByText, findAllByTestId, findByTestId, unmount } = render(
      SourceViewer,
      {
        props: {
          address: '0x00',
  ...
  it('scrolls to active line', async () => {
    ...
    await wrapper.setProps({
      activeStep: { address: '0x01', line: 3, step: {} },
    });
  ...
  it('emits nav:navigateToLine event on active line click', async () => {
    ...
    expect(wrapper.emitted()).toHaveProperty('nav:navigateToLine', [
      [{ line: 1, address: '0x01' }],
    ]);
    wrapper.unmount();
  });
