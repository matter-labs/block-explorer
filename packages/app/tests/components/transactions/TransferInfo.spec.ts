...
describe('TransferInfo:', () => {
  ...
  it('renders component properly', async () => {
    ...
    expect(
      wrapper.findComponent(RouterLinkStub).props().to.params.address,
    ).toBe('0x6c10d9C1744F149D4B17660E14FaA247964749c7');
  ...
  describe('when L1 explorer url is not set', () => {
    ...
    it('renders L1 address as text instead of a link', async () => {
      ...
      expect(wrapper.find('span')?.text()).toBe('From');
      expect(wrapper.findAll('span.address')[0].text()).toEqual(
        '0x6c10d9c1744...49c7',
      );
      expect(wrapper.find('.copy-btn')).toBeTruthy();
      expect(wrapper.find('.transactions-data-link-network')?.text()).toBe(
        'L1',
      );
