  ...
  logs: [
    {
      ...
      transactionHash:
        '0x9c526cc47ca2d3f72b7997a61d890d72951a283fa05d08df058ff8a629cffa3c',
      transactionIndex: '0',
    },
    {
...
describe('Transaction info table', () => {
  ...
  it('renders verified transaction properly', async () => {
    ...
    expect(txHash.find('.displayed-string').text()).toBe(
      '0x9c526cc47ca...ff8a629cffa3c',
    );
    ...
    expect(l2StatusBadgeTitle.text()).toBe(
      i18n.global.t('general.l2NetworkName'),
    );
    expect(l2StatusBadgeValue.text()).toBe(
      i18n.global.t('transactions.statusComponent.processed'),
    );
    expect(l1StatusBadgeTitle.text()).toBe(
      i18n.global.t('general.l1NetworkName'),
    );
    ...
    expect(l1StatusBadgeValueMobile.text()).toBe(
      i18n.global.t('transactions.statusComponent.executed'),
    );
    ...
    const transfers = tokensTransferred
      .findAll('.transfer-container')
      .map((row) => ({
    ...
    expect(inputData.find('.displayed-string').text()).toBe(
      '0xa9059cbb000...000000000000c',
    );
    expect(
      `${value.find('.token-amount').text()} ${fee.find('.token-symbol').text()}`,
    ).toBe('0 ETH');
    expect(
      `${fee.find('.token-amount').text()} ${fee.find('.token-symbol').text()}`,
    ).toBe('0.0014447025 ETH');
    ...
    const [
      ...
      .findAll('tbody .transaction-info-field-tooltip')
      .map((e) => e.text());
    expect(txHashTooltip).toBe(
      i18n.global.t('transactions.table.transactionHashTooltip'),
    );
    expect(statusTooltip).toBe(
      i18n.global.t('transactions.table.statusTooltip'),
    );
    ...
    expect(tokensTransferredTooltip).toBe(
      i18n.global.t('transactions.table.tokensTransferredTooltip'),
    );
    expect(inputDataTooltip).toBe(
      i18n.global.t('transactions.table.inputDataTooltip'),
    );
    ...
    expect(gasLimitAndUsedTooltip).toBe(
      i18n.global.t('transactions.table.gasLimitAndUsedTooltip'),
    );
    expect(gasPerPubdataTooltip).toBe(
      i18n.global.t('transactions.table.gasPerPubdataTooltip'),
    );
    expect(nonceTooltip).toBe(i18n.global.t('transactions.table.nonceTooltip'));
    expect(createdAtTooltip).toBe(
      i18n.global.t('transactions.table.createdTooltip'),
    );
  });
  it('renders indexing transaction status', async () => {
    ...
    expect(indexingBadge.text()).toBe(
      i18n.global.t('transactions.statusComponent.indexing'),
    );
  });
  it('renders failed transaction status', async () => {
    ...
    expect(badges[0].text()).toBe(
      i18n.global.t('transactions.statusComponent.failed'),
    );
  ...
  it('renders L1/L2 state properly', () => {
    ...
    const networks = transferContainer.findAll(
