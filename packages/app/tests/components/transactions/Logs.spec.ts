...
describe('Logs', () => {
  ...
  it('render components based on input', () => {
    ...
    expect(
      container.querySelector('.address-value-container')?.textContent,
    ).toBe('0x000000000000000000000000000000000000800A');
    expect(
      container
        .querySelector('.log-interface')
    ...
    expect(
      topicItem[0].querySelector('.topic-index-container')?.textContent,
    ).toBe('0');
    expect(topicItem[0].querySelector('.convert-dropdown-container')).toBe(
      null,
    );

    expect(
      topicItem[1].querySelector('.topic-index-container')?.textContent,
    ).toBe('1');
    expect(
      topicItem[1].querySelector('.convert-dropdown-container'),
    ).toBeTruthy();

    expect(
      topicItem[2].querySelector('.topic-index-container')?.textContent,
    ).toBe('2');
    expect(
      topicItem[2].querySelector('.convert-dropdown-container'),
    ).toBeTruthy();

    expect(container.querySelector('.hash-label-component')?.textContent).toBe(
      '1',
    );
  });
  ...
  describe('Pagination', () => {
    ...
    it('renders page according to route page query param', () => {
      ...
      expect(
        container.querySelector('.address-value-container')?.textContent,
      ).toBe('0xF44095b5f84c560011B901e0DeE5EB7DdF96fE8D');
