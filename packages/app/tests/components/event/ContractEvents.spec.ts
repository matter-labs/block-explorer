...
const contractEvent = {
  ...
  blockHash:
    '0x58b1af00d17caee971b3d72fb95d769331105d1a54cae586b955639ce6419bc1',
  blockNumber: 12,
...
describe('ContractEvents', () => {
  ...
  it('renders header properly', () => {
    ...
    expect(container.querySelectorAll('.table-head-col')[0].textContent).toBe(
      'txn hash',
    );
    expect(container.querySelectorAll('.table-head-col')[1].textContent).toBe(
      'method',
    );
    expect(container.querySelectorAll('.table-head-col')[2].textContent).toBe(
      'logs',
    );
  ...
  it('renders table row data based on input', () => {
    ...
    expect(
      container.querySelector('.first-col .hash-label-component')?.textContent,
    ).toBe(
      '0x7ca934c36aec488cdfacaf660a9257f471d5207a73467849f677e6d0502696e7',
    );
    expect(container.querySelector('.first-col > span')?.textContent).toBe(
      ' #12',
    );
    getByText('Transfer');
    expect(
      container.querySelector('.last-col .hash-label-component')?.textContent,
    ).toBe(
      '0x7ca934c36aec488cdfacaf660a9257f471d5207a73467849f677e6d0502696e7',
    );
    expect(container.querySelector('.topic-index-container')?.textContent).toBe(
      '0',
    );
    ...
    expect(
      container.querySelector('.data-container span')?.childNodes[0].nodeValue,
    ).toBe('value: ');
    expect(
      container.querySelector('.expandable-text-content')?.textContent,
    ).toBe('1');
    expect(container.querySelector('.expand-button')?.textContent).toBe(
      'Show More',
    );
  ...
  it('renders links properly', async () => {
    ...
    expect(routerLinks[3].props().to.params.address).toBe(
      '0x6cC8cf7f6b488C58AA909B77E6e65c631c204784',
    );

    expect(routerLinks[4].props().to.name).toBe('address');
    expect(routerLinks[4].props().to.params.address).toBe(
  ...
  it("shows 'Hex' type by default if SC is not verified", () => {
    ...
    expect(container.querySelectorAll('.selected-option')[0].textContent).toBe(
      'Hex',
    );
    expect(container.querySelectorAll('.selected-option')[1].textContent).toBe(
      'Hex',
    );
    expect(container.querySelectorAll('.selected-option')[2].textContent).toBe(
      'Hex',
    );
