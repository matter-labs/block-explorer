...
describe('Table', () => {
  ...
  it('shows not found message when there are no batches and loading is false', () => {
    ...
    expect(wrapper.find('.batches-not-found').text()).toContain(
      'No batches found.',
    );
  });
  it('renders batch index link', () => {
    ...
    expect(wrapper.findAllComponents(RouterLinkStub)[0].props().to.name).toBe(
      'batch',
    );
    expect(
      wrapper.findAllComponents(RouterLinkStub)[0].props().to.params.id,
    ).toBe('2');
  });
  ...
  it('renders batch size link', () => {
    ...
    expect(wrapper.findAllComponents(RouterLinkStub)[1].props().to.name).toBe(
      'batch',
    );
    expect(
      wrapper.findAllComponents(RouterLinkStub)[1].props().to.params.id,
    ).toBe('2');
  });
});
