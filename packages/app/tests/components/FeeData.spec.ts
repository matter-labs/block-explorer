...
describe('FeeToken', () => {
  ...
  it('displays the initial fee details when expanded', async () => {
    ...
    expect(
      container.querySelector('.details-data-container .details-title')
        ?.textContent,
    ).toBe('Initial:');
    expect(
      container.querySelector('.details-data-container .token-amount')
        ?.textContent,
    ).toBe('0.010000000001');
  });
  ...
  it('displays the refunds details when expanded', async () => {
    ...
    expect(
      container.querySelector(
        '.details-data-container:nth-child(2) .details-title',
      )?.textContent,
    ).toBe('Refunded:');
    expect(
      container.querySelector(
        '.details-data-container:nth-child(2) .token-amount',
      )?.textContent,
    ).toBe('0.000000000001');
  });
  ...
  describe('when transaction fee is not paid by paymaster', () => {
    ...
    it('displays the refunds when expanded', async () => {
      ...
      await fireEvent.click(container.querySelector('.toggle-button')!);
      const transfers = container.querySelectorAll(
        '.fee-transfers-container .transfer-container',
      );
      expect(transfers.length).toBe(1);
      const transferInfo = transfers[0].querySelectorAll(
        '.transfer-info-container',
      );
      expect(transferInfo[0].querySelector('.address span')?.textContent).toBe(
        '0x00000000000...8001',
      );
      expect(transferInfo[0].querySelector('.paymaster-label')).toBe(null);
      expect(transferInfo[1].querySelector('.address span')?.textContent).toBe(
        '0x9adEe6D82c0...1163',
      );
      expect(transferInfo[1].querySelector('.paymaster-label')).toBe(null);
      expect(
        container.querySelectorAll('.transfer-amount-container span')[0]
          ?.textContent,
      ).toBe('for');
      expect(
        container.querySelectorAll('.transfer-amount-container span')[1]
          ?.textContent,
      ).toBe('0.000000000001');
      expect(
        container.querySelectorAll('.transfer-amount-container span')[2]
          ?.textContent,
      ).toBe('ETH');
    });
  ...
  describe('when transaction fee is paid by paymaster', () => {
    it('display paid by paymaster label when fee is paid by paymaster', async () => {
      ...
      expect(
        container.querySelector('.payed-by-paymaster-label')?.textContent,
      ).toBe('Paid by Paymaster');
    });
    ...
    it('displays the refunds when expanded', async () => {
      ...
      expect(
        transferInfo[1].querySelector('.paymaster-label')?.textContent,
      ).toBe('Paymaster');
      expect(
        container.querySelectorAll('.transfer-amount-container span')[0]
          ?.textContent,
    ...
    it("displays the 'Why Paymaster Refunded?' and 'What is Paymaster' links when expanded", async () => {
      ...
      expect(refundedLink?.textContent).toBe(
        'Why is Paymaster being refunded?',
      );
