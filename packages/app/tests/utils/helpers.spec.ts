...
describe('helpers:', () => {
  ...
  it('returns utc string from ISO string', () => {
    expect(utcStringFromISOString('2022-04-08T18:21:14.362648Z')).toBe(
      '2022-04-08 18:21:14 UTC',
    );
  });

  it('returns ISO string from unix timestamp', () => {
    expect(ISOStringFromUnixTimestamp(1645606700)).toBe(
      '2022-02-23T08:58:20.000Z',
    );
  });
  ...
  it('returns local date from ISO string', () => {
    const result = format(
      new Date('2022-04-08T18:21:14.362648Z'),
      'yyyy-MM-dd HH:mm',
    );
