...
describe('highlight:', () => {
  it('highlights search text', () => {
    expect(highlight('any text', 'te')).toEqual(
      'any <mark class="mark">te</mark>xt',
    );
  });

  it('highlights only a single hit', () => {
    expect(highlight('any text any text', 'te')).toEqual(
      'any <mark class="mark">te</mark>xt any text',
    );
  });
  ...
  it('escapes special html symbols', () => {
    expect(highlight(`any <>"'& text`, 'text')).toEqual(
      'any &lt;&gt;&quot;&#039;&amp; <mark class="mark">text</mark>',
    );
  });
  ...
  it('handles multiple words', () => {
    expect(highlight('any random text', 'random te')).toEqual(
      'any <mark class="mark">random te</mark>xt',
    );
  });
});
