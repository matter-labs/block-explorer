...
describe('formatters:', () => {
  ...
  it('returns formatted Decimals data', () => {
    ...
    expect(
      formatHexDecimals(
        '000000000000000000000000e594ae1d7205e8e92fb22c59d040c31e1fcd139d',
        'Dec',
      ),
    ).toBe('1310674564365545227262193377675158530585864770461');
    expect(
      formatHexDecimals(
        '000000000000000000000040011002100000000000000000000000e501100197',
        'Dec',
      ),
    ).toBe('93542170771540876828785835717874066337266490409367');
  });
  describe('convert:', () => {
    const token = {
    ...
    it('return correct price', () => {
      expect(
        convert('0x56bc75e2d63100000', token, token.usdPrice.toString()),
      ).toBe('350000.0');
    });
  ...
  describe('numberToHexString', () => {
    ...
    it('returns hex str for the specified bigint', () => {
      expect(numberToHexString(BigInt('1000000000000000000000000'))).toBe(
