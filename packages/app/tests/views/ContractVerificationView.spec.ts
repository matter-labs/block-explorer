...
describe('ContractVerificationView:', () => {
  ...
  it('has correct title', async () => {
    expect(
      i18n.global.t(
        routes.find((e) => e.name === 'contract-verification')?.meta
          ?.title as string,
      ),
    ).toBe('Smart Contract Verification');
  });
  it('uses contract address from query', async () => {
    ...
    expect(
      (wrapper.find('#contractAddress').element as HTMLInputElement).value,
    ).toBe('0x5c221e77624690fff6dd741493d735a17716c26b');
  });
  ...
  it('shows single file verification when vyper single file verification is selection', async () => {
    ...
    await wrapper
      .find(`[aria-labelledby="compilerType"] > li:nth-child(3)`)
      .trigger('click');
    expect(wrapper.find('#sourceCode').exists()).toBe(true);
    expect(wrapper.find('.multi-file-verification').exists()).toBe(false);
  });
  it('shows multi file verification when solidity MFV was selected', async () => {
    ...
    await wrapper.find('#compilerType').trigger('click');
    await wrapper
      .find(`[aria-labelledby="compilerType"] > li:nth-child(2)`)
      .trigger('click');
  ...
  it('shows multi file verification when vyper MFV was selected', async () => {
    ...
    await wrapper.find('#compilerType').trigger('click');
    await wrapper
      .find(`[aria-labelledby="compilerType"] > li:nth-child(4)`)
      .trigger('click');
    expect(wrapper.find('#sourceCode').exists()).toBe(false);
    expect(wrapper.find('.multi-file-verification').exists()).toBe(true);
  });
  ...
  it('shows default error text when error is true', async () => {
    ...
    expect(wrapper.find('.error-alert').text()).toBe(
      i18n.global.t('contractVerification.form.unknownError'),
    );
    mock.mockRestore();
  });
  it('shows compilation errors', async () => {
    ...
    expect(wrapper.find('.compilation-error-log pre').text()).toBe(
  ...
  it("doesn't show contract path input when zkSolc version lower than v1.3.6 selected", async () => {
    ...
    await wrapper
      .find(`[aria-labelledby="zkCompilerVersion"] > li:nth-child(1)`)
      .trigger('click');
  ...
  it('renders solidity flattener link', async () => {
    ...
    expect(wrapper.findAll('.flattener-info a')[0].text()).toEqual(
      'solidity-flattener',
    );
  ...
  it('renders solidity flattery link', async () => {
    ...
    expect(wrapper.findAll('.flattener-info a')[1].text()).toEqual(
      'SolidityFlattery',
    );
  ...
  it('resets uploaded files block when clicking on clear button', async () => {
    ...
    await fireEvent.click(
      container.querySelector(
        `[aria-labelledby="compilerType"] > li:nth-child(2)`,
      )!,
    );
    ...
    expect(container.querySelectorAll('.file-preview-container')).toHaveLength(
      1,
    );
  ...
  it('resets main file when clicking on clear button', async () => {
    ...
    expect(
      container.querySelector('.multi-file-verification .toggle-button'),
    ).toBeNull();
    ...
    expect(
      container.querySelector('.multi-file-verification .toggle-button')
        ?.textContent,
    ).toBe('Choose Main File');
    const button = container.querySelector(
      '.multi-file-verification .toggle-button',
    );
    await fireEvent.click(button!);
    const li = container.querySelectorAll(
      '.multi-file-verification .options-list-item',
    );
