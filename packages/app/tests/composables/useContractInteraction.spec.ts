...
import useContractInteraction, {
  PAYABLE_AMOUNT_PARAM_NAME,
...
describe('useContractInteraction:', () => {
  ...
  describe('readFunction:', () => {
    ...
    it('sets isRequestFailed to true when request failed', async () => {
      ...
      const mock = vi
        .spyOn(ethers.Contract.prototype, 'transfer')
        .mockImplementation(mockTransfer);
      const { isRequestFailed, errorMessage, readFunction } =
  ...
  describe('writeFunction:', () => {
    ...
    it('sets isRequestFailed to true when request failed', async () => {
      ...
      const mock = vi
        .spyOn(ethers.Contract.prototype, 'transfer')
        .mockImplementation(mockTransfer);
      const { isRequestFailed, errorMessage, writeFunction } =
        useContractInteraction();
      await writeFunction(
        '0x0cc725e6ba24e7db79f62f22a7994a8ee33adc1b',
        abiFragment,
