...
export const useContractEventsMock = (params: any = {}) => {
  const mockContractEvent = vi
    .spyOn(useContractEvents, 'default')
    .mockReturnValue({
      total: ref(0),
      collection: ref([]),
      getCollection: () => vi.fn(),
...
export const useWalletMock = (params: any = {}) => {
  const mockWallet = vi.spyOn(useWallet, 'default').mockReturnValue({
    ...
    getL2Signer: vi.fn(async () => ({
      getAddress: async () => '0x000000000000000000000000000000000000800A',
    })),
    ...params,
  });
  return mockWallet;
...
export const useContractInteractionMock = (params: any = {}) => {
  const mockContractInteraction = vi
    .spyOn(useContractInteractionFactory, 'default')
    .mockReturnValue({
...
export const useTokenLibraryMock = (params: any = {}) => {
  const mockToken = vi
    .spyOn(useTokenLibraryMockFactory, 'default')
    .mockReturnValue({
...
export const useTransactionsMock = (params: any = {}) => {
  const mockTransactions = vi
    .spyOn(useTransactions, 'default')
    .mockReturnValue({
      data: ref([]),
      load: () => vi.fn(),
      pending: ref(false),
