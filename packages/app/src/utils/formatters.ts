...
export function stringFromAsciiArray(asciiArray: number[]) {
  return asciiArray.reduce(
    (acc, currentValue) => acc + String.fromCharCode(currentValue),
    '',
  );
}
...
export function convert(
  ...
  if (token && value) {
    return formatValue(
      (
        (BigInt(value) *
          BigInt(Math.round(+parseFloat(tokenPrice).toFixed(6) * 1000000))) /
        BigInt(1000000)
      ).toString(),
      token.decimals,
    );
...
export const numberToHexString = (num: number | bigint) =>
  `0x${num.toString(16)}`;

export function formatPricePretty(
  amount: BigNumberish,
  decimals: number,
  usdPrice: string,
) {
  ...
  const leadingZeroes =
    price.toString().split('.')[1]?.match(/^0*/)?.[0].length || 0;
