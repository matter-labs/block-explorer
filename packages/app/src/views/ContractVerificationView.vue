  ...
  <div>
    ...
    >
      <Alert type="notification" class="full-grid-width">
        <i18n-t
          scope="global"
          keypath="contractVerification.resources.title"
          tag="span"
        >
          <template #hardhat>
            <a
              href="https://docs.zksync.io/build/tooling/hardhat/hardhat-zksync-verify.html"
              target="_blank"
            >
      ...
      <div v-if="compilationErrors.length" class="compilation-error-container">
        ...
        <ExpandableText class="compilation-error-log">
          <template #default>
            <pre v-for="(item, index) in compilationErrors" :key="index">{{
              item
            }}</pre>
          </template>
      ...
      <Alert
        v-else-if="isRequestFailed || contractVerificationErrorMessage"
        class="error-alert"
      >
        {{
      ...
      <h3 class="form-subheading">
        {{ t('contractVerification.form.contractAddress.title') }}
      </h3>
      <FormItem id="contractAddress" class="half-grid-width">
        <Input
          ...
          :placeholder="
            t('contractVerification.form.contractAddress.placeholder')
          "
          :error="
            v$.contractAddress.$error
              ? v$.contractAddress.$errors[0] &&
                v$.contractAddress.$errors[0].$message.toString()
              : undefined
          "
          v-model="form.contractAddress"
        />
        <template #underline>{{
          t('contractVerification.form.contractAddress.underline')
        }}</template>
      </FormItem>

      <h3 class="form-subheading">
        {{ t('contractVerification.form.compilationInfo') }}
      </h3>
      <FormItem
        id="compilerType"
        :label="t('contractVerification.form.compilerType.label')"
      >
        <Dropdown
          ...
          :formatter="
            (val) => t(`contractVerification.form.compilerType.${val}`)
          "
          @update:model-value="onCompilationTypeChange"
        />
        <template #underline>{{
          t('contractVerification.form.compilerType.underline')
        }}</template>
      </FormItem>
      <div class="contract-dropdowns-container half-grid-width">
        <FormItem
          ...
          :label="
            t(
              `contractVerification.form.${selectedZkCompiler.name}Version.label`,
            )
          "
        ...
        >
          <a
            ...
            >{{
              t(
                `contractVerification.form.${selectedZkCompiler.name}Version.details`,
              )
            }}</a
          >
          <Dropdown
            ...
            :error="
              v$.zkCompilerVersion.$error || selectedZkCompiler.isRequestFailed
                ? t(
                    `contractVerification.form.${selectedZkCompiler.name}Version.error`,
                  )
                : ''
            "
          />
        ...
        <FormItem
          id="compilerVersion"
          :label="
            t(`contractVerification.form.${selectedCompiler.name}Version.label`)
          "
          class="label-inline-block"
        >
          <CheckBoxInput
          ...
          <Dropdown
            ...
            :error="
              v$.compilerVersion.$error || selectedCompiler.isRequestFailed
                ? t(
                    `contractVerification.form.${selectedCompiler.name}Version.error`,
                  )
                : ''
            "
          />
      ...
      <FormItem
        tag="fieldset"
        :label="t('contractVerification.form.optimizationUsed.label')"
        label-tag="legend"
      >
        <div
          class="grid w-max grid-cols-2 items-center gap-4"
          :data-testid="$testId.optimizationRadioButtons"
        >
          <RadioInput
            id="optimization-yes"
            name="optimization"
        ...
        <template #underline>{{
          t('contractVerification.form.optimizationUsed.underline')
        }}</template>
      </FormItem>
      ...
      <FormItem
        id="contractName"
        :label="t('contractVerification.form.contractName.label')"
      >
        <Input
          id="contractName"
          type="text"
          ...
          :error="
            v$.contractName.$error
              ? v$.contractName.$errors[0] &&
                v$.contractName.$errors[0].$message.toString()
              : undefined
          "
        ...
        <template #underline>{{
          t(
            `contractVerification.form.contractName.${selectedCompiler.name}Underline`,
          )
        }}</template>
      </FormItem>
      <FormItem
        v-if="isFilePathRequired"
        id="contractPath"
        :label="t('contractVerification.form.contractPath.label')"
      >
        <Input
          id="contractPath"
          type="text"
          ...
          :placeholder="
            t(
              `contractVerification.form.contractPath.${selectedCompiler.name}Placeholder`,
            )
          "
          :error="
            v$.contractPath.$error
              ? v$.contractPath.$errors[0] &&
                v$.contractPath.$errors[0].$message.toString()
              : undefined
          "
          v-model="form.contractPath"
        />
        <template #underline>{{
          t('contractVerification.form.contractPath.underline')
        }}</template>
      </FormItem>
      <FormItem
        v-if="isSingleFile"
        ...
        :label="
          t(
            `contractVerification.form.sourceCode.${selectedCompiler.name}Label`,
          )
        "
      >
        <SolidityEditor
          ...
          :error="
            v$.sourceCode.$error
              ? v$.sourceCode.$errors[0] &&
                v$.sourceCode.$errors[0].$message.toString()
              : undefined
          "
        ...
        >
          ...
          <template #flattery>
            <a
              href="https://github.com/DaveAppleton/SolidityFlattery"
              target="_blank"
            >
              {{
      ...
      <MultiFileVerification
        ...
        :error-files="
          v$.files.$error
            ? v$.files.$errors[0] && v$.files.$errors[0].$message.toString()
            : ''
        "
        :error-main-file="
          v$.mainFileName.$error
            ? v$.mainFileName.$errors[0] &&
              v$.mainFileName.$errors[0].$message.toString()
            : ''
        "
        class="full-grid-width"
      />
      ...
      >
        <Input
          ...
          :disabled="isRequestPending"
          :placeholder="
            t('contractVerification.form.constructorArguments.placeholder')
          "
    ...
    <SuccessScreen
      ...
      :contract-address="form.contractAddress as Address"
    />
  </div>
</template>
...
const selectedCompilationType = ref(
  CompilationTypeOptionsEnum.soliditySingleFile,
);
const isSingleFile = computed(() =>
  [
    CompilationTypeOptionsEnum.soliditySingleFile,
    CompilationTypeOptionsEnum.vyperSingleFile,
  ].includes(selectedCompilationType.value),
);
  ...
  if (compiler === CompilerEnum.solc) {
    return {
      ...
      versions: compilerInfo.versions?.filter((version) =>
        isZkVMSolcCompiler.value
          ? version.startsWith(zkVMVersionPrefix)
          : !version.startsWith(zkVMVersionPrefix),
      ),
    };
  }
...
const selectedZkCompilerVersion = ref(
  selectedZkCompiler.value.versions[
    selectedZkCompiler.value.versions.length - 1
  ] || '',
);
const selectedCompilerVersion = ref(
  selectedCompiler.value.versions[selectedCompiler.value.versions.length - 1] ||
    '',
);
...
const isFilePathRequired = computed(() => {
  ...
  return (
    selected.localeCompare(target, undefined, {
      numeric: true,
      sensitivity: 'base',
    }) >= 0
  );
...
const v$ = useVuelidate(
  {
    contractAddress: {
      required: withI18nMessage(required, {
        messagePath: () =>
          'contractVerification.form.contractAddress.validation.required',
      }),
      isContractAddress: withI18nMessage(isAddress, {
        messagePath: () =>
          'contractVerification.form.contractAddress.validation.not_valid',
      }),
    },
    contractName: {
      required: withI18nMessage(required, {
        messagePath: () =>
          'contractVerification.form.contractName.validation.required',
      }),
    },
    contractPath: {
      required: withI18nMessage(
        ...
        {
          messagePath: () =>
            'contractVerification.form.contractPath.validation.required',
        },
      ),
    },
    zkCompilerVersion: {
      required: withI18nMessage(required, {
        messagePath: () =>
          `contractVerification.form.${selectedZkCompiler.value.name}Version.validation.required`,
      }),
    },
    compilerVersion: {
      required: withI18nMessage(required, {
        messagePath: () =>
          `contractVerification.form.${selectedCompiler.value.name}Version.validation.required`,
      }),
    },
    sourceCode: {
      required: withI18nMessage(
        ...
        {
          messagePath: () =>
            'contractVerification.form.sourceCode.validation.required',
        },
      ),
    },
    mainFileName: {
      required: withI18nMessage(
        ...
        {
          messagePath: () =>
            'contractVerification.multiFileVerification.mainFile.validation.required',
        },
      ),
    },
    files: {
      required: withI18nMessage(
        ...
        {
          messagePath: () =>
            'contractVerification.multiFileVerification.files.validation.required',
        },
      ),
    },
  },
...
async function submitForm() {
  form.value.zkCompilerVersion =
    selectedZkCompilerVersion.value || defaultValues.value.zkCompilerVersion;
  form.value.compilerVersion =
    selectedCompilerVersion.value || defaultValues.value.compilerVersion;
  ...
  const commonData = {
    codeFormat:
      ContractVerificationCodeFormatEnum[selectedCompilationType.value],
  ...
  if (isSingleFile.value) {
    ...
    await requestVerification({
      ...
      sourceCode: {
        language:
          selectedCompiler.value.name === CompilerEnum.solc
            ? 'Solidity'
            : 'Vyper',
        sources,
        settings: {
