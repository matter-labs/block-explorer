<template>
  <SystemAlert v-if="isIndexerDelayed">
    ...
    <span
      v-else
      v-html="t('systemAlert.indexerDelayed', { indexerDelayInHours })"
    />
  </SystemAlert>
</template>
...
const { load: getLatestIndexedBlocks, data: latestIndexedBlocks } =
  useBlocks(context);
...
const indexerDelay = computed(() => {
  ...
  const latestIndexedBlockDate = new Date(
    latestIndexedBlocks.value[0].timestamp,
  );
  const delay = latestBlockDate.getTime() - latestIndexedBlockDate.getTime();
  return delay;
});

const indexerDelayInHours = computed(() =>
  (indexerDelay.value / (1000 * 60 * 60)).toFixed(1),
);
...
const latestTPS = computed(() => {
  ...
  const numberOfTransactions = latestIndexedBlocks.value.reduce(
  ...
  const firstBlockDate = new Date(
    latestIndexedBlocks.value[latestIndexedBlocks.value.length - 1].timestamp,
  );
  const numberOfSeconds = Math.floor(
    (latestBlockDate.getTime() - firstBlockDate.getTime()) / 1000,
  );
