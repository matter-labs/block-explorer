<template>
  ...
  >
    <template v-if="batches?.length || loading" #table-head>
      ...
      <TableHeadColumn v-if="columns.includes('txnBatch')">{{
        t('batches.table.txnBatch')
      }}</TableHeadColumn>
      <TableHeadColumn v-if="columns.includes('size')">{{
        t('batches.table.size')
      }}</TableHeadColumn>
    ...
    <template #table-row="{ item }: { item: any }">
      <TableBodyColumn
        ...
        :data-heading="t('batches.table.status')"
      >
        <div class="status-badge-container">
          <Badge color="dark-success" :data-testid="$testId.statusBadge">
            <template #default>
              {{
                te(`batches.status.${item.status}`)
                  ? t(`batches.status.${item.status}`)
                  : item.status
      ...
      <TableBodyColumn
        v-if="columns.includes('txnBatch')"
        :data-heading="t('batches.table.txnBatch')"
      >
        <router-link :to="{ name: 'batch', params: { id: item.number } }">
          #{{ item.number }}
        </router-link>
      </TableBodyColumn>
      <TableBodyColumn
        v-if="columns.includes('size')"
        :data-heading="t('batches.table.size')"
      >
        <router-link
          :to="{
            name: 'batch',
            params: { id: item.number, scrollToTxns: true },
          }"
        >
      ...
      <TableBodyColumn
        ...
        :data-heading="t('batches.table.age')"
      >
