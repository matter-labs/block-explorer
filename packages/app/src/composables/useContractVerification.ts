...
class ContractVerificationError extends Error {
  constructor(
    message: string,
    public readonly response: ContractVerificationStatusResponse,
  ) {
    super(message);
...
export default (context = useContext()) => {
  ...
  const compilerVersions = ref<Record<Compiler, CompilerState>>(
    Object.values(CompilerEnum).reduce(
      (acc: Record<string, CompilerState>, compiler: Compiler) => {
        acc[compiler] = {
          name: compiler,
  ...
  const contractVerificationStatus = ref<null | ContractVerificationStatus>(
    null,
  );
  ...
  const getStatusById = async (
    ...
    const response: {
      ...
      `${context.currentNetwork.value.verificationApiUrl}/contract_verification/${id}`,
    );
    if (response.error) {
  ...
  const requestVerification = async (data: ContractVerificationData) => {
    ...
    try {
      ...
      const { sourceCode, zkCompilerVersion, compilerVersion, ...payload } =
        data;
      const response = await $fetch(
        `${context.currentNetwork.value.verificationApiUrl}/contract_verification`,
        {
          method: 'POST',
          body: {
            ...payload,
            ...
            ...(!isSolidityContract && {
              ...
              ...(typeof sourceCode !== 'string' && {
                sourceCode: Object.keys(sourceCode.sources).reduce(
                  (acc: Record<string, string>, filename: string) => {
                    acc[filename.replace('.vy', '')] =
  ...
  const requestCompilerVersions = async (compiler: Compiler) => {
    ...
    try {
      ...
      compilerVersions.value[compiler].versions = result.sort(
        (a: string, b: string) => {
